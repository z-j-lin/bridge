...
contract MockProposalLogic is GovernanceProposal, DSTest {
    function execute(address self) public virtual override returns (bool) {
        return _execute();
    }
    ...
    function _execute() internal virtual returns (bool) {
        ...
        (bool success, bytes memory data) = _logic.call(
...
contract MockProposalChangeGovernanceManagerStorage is
    GovernanceProposal,
    DSTest
{
    function execute(address self) public virtual override returns (bool) {
...
contract MockProposalLogicWithAllowedProposals is GovernanceProposal, DSTest {
    function execute(address self) public virtual override returns (bool) {
        emit log_named_address(
            "MockProposalLogicWithAllowedProposals: msg.sender",
            msg.sender
        );
        ...
        (bool success, bytes memory data) = _logic.call(
            abi.encodeWithSignature("dummyNoDelegate()")
        );
        require(success, "GovernanceManager: CALL FAILED to execute proposal");
        emit log_named_bytes("Executed successful", data);
        return success;
...
contract MockProposalLogicWithOutAllowedProposals is
    GovernanceProposal,
    DSTest
{
    function execute(address self) public virtual override returns (bool) {
...
contract NoGovernanceDelegateCall is DSTest {
    function dummyNoDelegate() public returns (bool) {
        emit log_named_address(
            "NoGovernanceDelegateCall: msg.sender",
            msg.sender
        );
        address _logic = 0x3A1148FE01e3c4721D93fe8A36c2b5C29109B6ae;
        (bool success, bytes memory data) = _logic.call(
...
contract MockProposalChangeGovernanceManagerStorageStake is
    GovernanceProposal,
    DSTest
{
    function execute(address self) public virtual override returns (bool) {
...
contract GovernanceManagerTest is DSTest {
    ...
    function testVoteAsStaker() public {
        ...
        MockProposalLogic logic = new MockProposalLogic();
        MockGovernanceOnlyAction dummy = new MockGovernanceOnlyAction(
    ...
    function testVoteAsMiner() public {
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            uint256 tokenID = minerStake.mintTo(
                address(user),
                11_220_000 * 10**18,
                1
            );
    ...
    function testSameUserVotesAsMinerAndStaker() public {
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            uint256 minerTokenId = minerStake.mintTo(
                address(users[i]),
                11_000_000 * 10**18,
                1
            );
            emit log_named_uint("miner token", minerTokenId);
            uint256 stakeTokenId = stakeNFT.mintTo(
                address(users[i]),
                11_000_000 * 10**18,
                1
            );
    ...
    function testFail_ShouldNotBeAbleToVoteOnSameBlockAsProposalCreation()
        public
    {
        (
            StakeNFT stakeNFT,
        ...
        // 112 200 000
        UserAccount user1 = newUserAccount(
            madToken,
            stakeNFT,
            governanceManager
        );
    ...
    function testFail_UserVoteWithNotOwnedPosition() public {
        ...
        UserAccount user2 = newUserAccount(
            madToken,
            stakeNFT,
            governanceManager
        );
        madToken.approve(address(stakeNFT), 200_000_000 * 10**18);
        uint256 tokenID1 = stakeNFT.mintTo(
            address(user1),
            100_200_000 * 10**18,
            1
        );
        uint256 tokenID2 = stakeNFT.mintTo(
            address(user2),
            12_000_000 * 10**18,
            1
        );
    ...
    function testVoteAndExecuteMultipleProposal() public {
        ...
        assertProposal(
            ...
            GovernanceStorage.Proposal(false, address(logic1), 0, 172800)
        );
        ...
        assertProposal(
            ...
            GovernanceStorage.Proposal(false, address(logic2), 0, 172800)
        );
    ...
    function testExecuteProposalThatModifyStorage() public {
        ...
        assertProposal(
            governanceManager.getProposal(proposalID),
            GovernanceStorage.Proposal(false, address(0x0), 0, 666)
        );
        user1.voteAsStaker(proposalID, tokenID);
        governanceManager.execute(proposalID);
    ...
    function testExecuteGovernanceAllowedProposal() public {
        ...
        emit log_named_address(
            "Address dummy delegate:",
            address(dummyDelegate)
        );
        uint256 proposalID = governanceManager.propose(address(logic));
        assertProposal(
            governanceManager.getProposal(proposalID),

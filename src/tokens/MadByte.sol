    ...
    Sigmoid
{
    ...
    function EthtoMB(uint256 poolBalance_, uint256 numEth_)
        public
        pure
        returns (uint256)
    {
    ...
    /// uint256).
    /// @param depositID The Id of the deposit
    function getDepositOwner(uint256 depositID)
        public
        view
        returns (address, BNAddress memory)
    {
        require(depositID <= _depositID, "MadByte: Invalid deposit ID!");
    ...
    /// @param amount_ The amount of Madbytes to be deposited
    /// Return The deposit ID of the deposit created
    function virtualMintDeposit(address to_, uint256 amount_)
        public
        onlyAdmin
        returns (uint256)
    {
    ...
    /// Return The deposit ID of the deposit created
    function mintDeposit(address to_, uint256 minMB_)
        public
        payable
        returns (uint256)
    {
    ...
    function _deposit(address to_, uint256 amount_) internal returns (uint256) {
        require(
            !_isContract(to_),
            "MadByte: Contracts cannot make MadBytes deposits!"
        );
        require(
            amount_ > 0,
        ...
        require(
            ...
            "MadByte: Burn failed during the deposit!"
        );
    ...
    // burning any token.
    function _virtualDeposit(address to_, uint256 amount_)
        internal
        returns (uint256)
    {
        require(
    ...
    function _EthtoMB(uint256 poolBalance_, uint256 numEth_)
        internal
        pure
        returns (uint256)
    {
    ...
    function _MBtoEth(
        ...
        return
            _min(poolBalance_, _fp(totalSupply_) - _fp(totalSupply_ - numMB_));
    }
}
